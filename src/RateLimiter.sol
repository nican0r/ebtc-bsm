...
/// https://raw.githubusercontent.com/smartcontractkit/ccip/refs/heads/ccip-develop/contracts/src/v0.8/ccip/libraries/RateLimiter.sol
contract RateLimiter is AuthNoOwner {
    ...
    mapping(address => TokenBucket) internal tokenBucket;

    constructor() {
        _initializeAuthority(GOVERNANCE);
    }

    function checkMint(uint256 requestTokens) external {
        TokenBucket storage s_bucket = tokenBucket[msg.sender];
    ...
    function setTokenBucketConfig(
        ...
        _validateTokenBucketConfig(config, false);
        _setTokenBucketConfig(tokenBucket[consumer], config);
    }

    function getTokenBucketState(address consumer) external view returns (TokenBucket memory) {
        return _currentTokenBucketState(tokenBucket[consumer]);
    }
